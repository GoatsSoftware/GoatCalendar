...
Le sujet impose un dépôt Git, une pipeline CI, une architecture aback de type microservices en couche, au moins deux services back, des tests unitaires, d'intégration, une couverture de code d'au moins 80%, et une qualité logicielle élevée. La base de données et le front web sont notés en bonus. Dans la version analysée, le projet contient trois services backend, un front web statique, une base de données MySQL. Les services Python sont publiés dans le l'index des librairies Python PyPi, l'application est conteneurisée et déployée avec Docker et déployée. SonarQube Cloud est utilisée pour vérifier la qualité du code de l'application.
...
\midrule
Dépôt Git & Le rapport contient le lien du dépôt GitHub du projet. \\
Pipeline CI & Lance l'analyse SonarCloud, exécute les tests unitaires, d'intégration et de bout-en-bout, et publie dans PyPI les services Python et les images dans DockerHub.  \\
Architecture en couches & Présente : routes FastAPI, services métier, repositories, schémas, models et DTO partagés. \\
...
Qualité logicielle & Ruff configuré dans les services Python, et SonarCloud configuré à la racine. \\
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Controller/Web} : fichiers \texttt{routes/*.py}. Ils exposent les endpoints FastAPI, injectent les dépendances, appelent la couche logique pour traiter la demande, et convertissent les erreurs, ressources en réponses HTTP.
  \item \textbf{Services} : fichiers \texttt{services/*.py}. Ils portent la logique métier, la sérialisation DTO et les règles de création/mise à jour/récupération de ressource(s).
  ...
  \item \textbf{Shared models} : fichiers \texttt{shared\_models/*.py}. Il centralise la déclaration des schémas SQLModel mappés aux tables SQL, modèles, énumérations, exceptions, DTO utilisés par les services back.
\end{itemize}
...
Le service d'authentification est une application FastAPI exposée sur le port 5001. Il dépend du package \texttt{database\_service} pour effectuer les requêtes dans la base de données.

Il s'occupe aussi de l'insertion d'un jeux de données de base.
...
Le service Board est une application FastAPI exposée sur le port 5002. Il contient le coeur métier de l'application, à savoir la gestion des boards.

Il dépend du package \texttt{database\_service} via la dépendance \texttt{get\_db\_session}, pour effectuer les requêtes dans la base de données.
Il dépend du package \texttt{auth\_service} via la dépendance \texttt{get\_current\_user}, pour sécuriser les endpoints et vérifier de l'authenticité de l'utilisateur d'après le token JWT transmis dans le header de la requête HTTP.
...
Ce package contient les différentes classes utilisés par les différents services back :
...
Chaque package Python utilise PDM avec \texttt{pyproject.toml} et \texttt{pdm.lock}. Les dépendances locales entre services sont relatives en développement. Les dépendances de production sont exportées dans les Dockerfiles avec \texttt{pdm export}. Le front ne nécessite pas de build Node : il est copié tel quel dans l'image Nginx.

\subsection{Qualité statique}

Ruff est utilisé par tous les packages back Python pour le formatage et le contrôle de la qualité du code. Les scripts PDM pour lancer diverses commandes ruff sont les suivantes :
...
La version finale contient plus de 100 tests Pytest, réparties dans les quatre packages backend :
...
Le seed créé un jeu de données de base pour permettre d'utiliser l'application directement :
...
Goat Calendar présente une base DevOps complète pour le cadre du projet : architecture microservices en couches pour la partie back, un front bonus, une base  de données MySQL, PyPi, DockerHub et Docker Compose pour le déploiement, des tests Pytest, Ruff et configuration SonarCloud. Le projet respecte les attentes principales du sujet et ajoute les deux bonus importants : interface web et base de données.
